...
const LeafletMap = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<main class="svelte-c2vcyt"><div class="input-group svelte-c2vcyt"><label class="svelte-c2vcyt"><input type="checkbox"${add_attribute("checked", showCustomers, 1)}> Customers</label> <label class="svelte-c2vcyt"><input type="checkbox"${add_attribute("checked", showForecast, 1)}> Forecast</label> <label class="svelte-c2vcyt"><input type="checkbox"${add_attribute("checked", showInventory, 1)}> Inventory</label> <label for="selectedDate" class="svelte-c2vcyt" data-svelte-h="svelte-166a445">Select Date:</label> <select id="selectedDate">${each(dateOptions, (date) => {
    return `<option${add_attribute("value", date, 0)}>${escape(date)}</option>`;
  })}</select></div> <div class="svelte-c2vcyt"${add_attribute("this", mapElement, 0)}></div> </main>`;
});
const Timeseries = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  Chart.register(LineElement, LinearScale, PointElement);
  let chart;
  return `<div class="container"><div class="inputs"><label for="startDate" data-svelte-h="svelte-a381xo">Start Date:</label> <input type="date" id="startDate" value="2024-05-01"> <label for="endDate" data-svelte-h="svelte-1gbtc26">End Date:</label> <input type="date" id="endDate" value="2024-05-30"></div> <canvas id="TimeSeries" width="200" height="50"${add_attribute("this", chart, 0)}></canvas></div>`;
});
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-ov818q_START -->${$$result.title = `<title>Content</title>`, ""}<meta name="description" content="Main content"><!-- HEAD_svelte-ov818q_END -->`, ""} <nav class="svelte-1ijii5c"><ul role="tablist" class="tabs-container svelte-1ijii5c">${each(tabs, ({ id, label }) => {
    return `<li class="svelte-1ijii5c"><button role="tab"${add_attribute("aria-selected", $activeTab === id, 0)} class="${["svelte-1ijii5c", $activeTab === id ? "selected" : ""].join(" ").trim()}">${escape(label)}</button> </li>`;
  })}</ul></nav> ${each(tabs, ({ id, label }) => {
    return `${$activeTab === id ? `<section class="content svelte-1ijii5c">${id === "map" ? `<div><h1 style="padding: 0px; margin: 1px; margin-top: 2px; margin-bottom: 1px; align: center " data-svelte-h="svelte-mhcad9">Inventory Quantities by Date</h1> ${validate_component(LeafletMap, "LeafletMap").$$render($$result, {}, {}, {})} </div>` : `${id === "timeSeries" ? `<div>${validate_component(Timeseries, "TimeSeries").$$render($$result, {}, {}, {})}</div> <div>${validate_component(Timeseriesmonthly, "TimeSeriesMonthly").$$render($$result, {}, {}, {})} </div>` : `${id === "correlations" ? `<h2>${escape(label)} Content</h2> <div>${validate_component(Correlation, "Correlation").$$render($$result, {}, {}, {})} </div>` : `${id === "plantcustomerdemand" ? `<h2>${escape(label)} Content</h2> <p>This is the content for the ${escape(label)} tab.</p>   <div><h1 data-svelte-h="svelte-111zjoq">Merged Data Summary</h1> ${mergedData.length > 0 ? `<table><thead data-svelte-h="svelte-5umbfl"><tr><th>Date</th> <th>Plant Key</th> <th>GIQ</th> <th>OSQ</th> <th>ITQ</th> <th>FQ</th> <th>Latitude</th> <th>Longitude</th> </tr></thead> <tbody>${each(mergedData, ({ Date, PlantKey, GIQ, OSQ, ITQ, FQ, lat, lon }) => {
      ...
      return `${file === selectedFile ? `<div><h3>${escape(file)}</h3> <ul>${each(keys, (key) => {
        return `<li class="svelte-1ijii5c">${escape(key)}</li>`;
