  ...
  return `${$$result.head += `<!-- HEAD_svelte-ov818q_START -->${$$result.title = `<title>Content</title>`, ""}<meta name="description" content="Main content"><!-- HEAD_svelte-ov818q_END -->`, ""} <nav class="svelte-3b8hzr"><ul role="tablist" class="tabs-container svelte-3b8hzr">${each(tabs, ({ id, label }) => {
    ...
    return `${$activeTab === id ? `<section class="content svelte-3b8hzr">${id === "map" ? `<div><h1 style="padding: 0px; margin: 1px; margin-top: 2px; margin-bottom: 1px; align: center " data-svelte-h="svelte-mhcad9">Inventory Quantities by Date</h1> ${validate_component(LeafletMap, "LeafletMap").$$render($$result, {}, {}, {})} </div>` : `${id === "timeSeries" ? `<div>${validate_component(Timeseries, "TimeSeries").$$render($$result, {}, {}, {})}</div> <div>${validate_component(Timeseriesmonthly, "TimeSeriesMonthly").$$render($$result, {}, {}, {})} </div>` : `${id === "correlations" ? `<h2>${escape(label)} Content</h2> <div>${validate_component(Correlation, "Correlation").$$render($$result, {}, {}, {})} </div>` : `${id === "plantcustomerdemand" ? `<h2>${escape(label)} Content</h2> <p>This is the content for the ${escape(label)} tab.</p>   <div><h1 data-svelte-h="svelte-111zjoq">Merged Data Summary</h1> ${mergedData.length > 0 ? `<table><thead data-svelte-h="svelte-5umbfl"><tr><th>Date</th> <th>Plant Key</th> <th>GIQ</th> <th>OSQ</th> <th>ITQ</th> <th>FQ</th> <th>Latitude</th> <th>Longitude</th> </tr></thead> <tbody>${each(mergedData, ({ Date, PlantKey, GIQ, OSQ, ITQ, FQ, lat, lon }) => {
      ...
      return `${file === selectedFile ? `<div><h3>${escape(file)}</h3> <ul>${each(keys, (key) => {
        return `<li class="svelte-3b8hzr">${escape(key)}</li>`;
